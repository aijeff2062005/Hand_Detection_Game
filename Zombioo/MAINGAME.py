...
import time
import itertools
from hand_control import HandController, HandData

# ── Hand Controller (background thread tự khởi động trong __init__) ──
try:
    hand = HandController(show_debug=True)
    _HAND_ENABLED = True
except RuntimeError as _e:
    print(f"[WARNING] Không khởi động được camera: {_e}")
    hand = None
    _HAND_ENABLED = False
...
# ── Gesture state – tránh trigger nhiều lần khi giữ cử chỉ ──
_prev_gesture   = "idle"      # gesture của frame trước
_gesture_lock   = {}          # {gesture: frames_remaining} – cooldown
_GESTURE_CD = {              # số frame khóa sau mỗi lần bật
    "jump"   : 35,            # ~0.4 giây @ 85 FPS
    "reload" : 60,            # ~0.7 giây
    "grenade": 25,
    "molotov": 25,
}
while run:
    ...
    # ── Cử chỉ tay → điều khiển (one-shot cho các action nhất thời) ─────────
    if _HAND_ENABLED and start_game and player.alive:
        _hd: HandData = hand.get_data()

        # Đếm ngược cooldown
        for _k in list(_gesture_lock):
            _gesture_lock[_k] -= 1
            if _gesture_lock[_k] <= 0:
                del _gesture_lock[_k]

        if _hd.hand_present:
            _g = _hd.gesture

            # Kiểm tra có phải gesture mới (vừa thay đổi) không
            _is_new = (_g != _prev_gesture)
            _prev_gesture = _g

            # ─ Di chuyển thuần: giữ gesture ─
            moving_left  = (_g in ("move_left",  "jump_left"))
            moving_right = (_g in ("move_right", "jump_right"))

            # ─ Bắn: giữ – cooldown do Soldier.shoot() tự xử lý ─
            shoot = (_g == "shoot")

            # ─ NHẢY tại chỗ: one-shot + cooldown ─
            if _g == "jump" and _is_new and "jump" not in _gesture_lock:
                player.jump = True
                _gesture_lock["jump"] = _GESTURE_CD["jump"]

            # ─ NHẢY TRÁI: di chuyển trái + jump one-shot ─
            if _g == "jump_left" and _is_new and "jump" not in _gesture_lock:
                player.jump = True
                _gesture_lock["jump"] = _GESTURE_CD["jump"]

            # ─ NHẢY PHẢI: di chuyển phải + jump one-shot ─
            if _g == "jump_right" and _is_new and "jump" not in _gesture_lock:
                player.jump = True
                _gesture_lock["jump"] = _GESTURE_CD["jump"]

            # ─ LỰU ĐẠN: one-shot ─
            if _g == "grenade" and _is_new and "grenade" not in _gesture_lock:
                grenade = True
                _gesture_lock["grenade"] = _GESTURE_CD["grenade"]
            elif _g != "grenade":
                grenade = False
                grenade_thrown = False

            # ─ MOLOTOV: one-shot ─
            if _g == "molotov" and _is_new and "molotov" not in _gesture_lock:
                molotov = True
                _gesture_lock["molotov"] = _GESTURE_CD["molotov"]
            elif _g != "molotov":
                molotov = False
                molotov_thrown = False

            # ─ NẠP ĐẠN: one-shot + cooldown ─
            if _g == "reload" and _is_new and "reload" not in _gesture_lock:
                player.ammo = player.start_ammo
                RELOAD.play()
                _gesture_lock["reload"] = _GESTURE_CD["reload"]

        else:
            # Không thấy tay → đặt lại trạng thái di chuyển
            _prev_gesture = "idle"
            moving_left   = False
            moving_right  = False
            shoot         = False

    # ── Sự kiện bàn phím (vẫn hoạt động song song) ───────────────────────────
...
# Dừng hand controller khi thoát game
if _HAND_ENABLED and hand is not None:
    hand.stop()
pygame.quit()
